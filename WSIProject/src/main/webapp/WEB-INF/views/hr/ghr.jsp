								...
								<ul class="nav nav-pills">
									...
									<select class="form-select" name="searchType">
											...
											<option value="empDept3"
												<c:if test="${searchType eq 'empDept3' }">selected</c:if>>직책</option>
									</select></li>
												...
												<tr onclick="location.href='/mypage/other?empNo=${hr.empNo }'">
													...
													<td>
														<div class="d-flex gap-2">
															<c:if test="${hr.empPos eq '직책 없음' }">
																<h5><span class="badge badge-soft-secondary">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq '파트장' }">
																<h5><span class="badge badge-soft-warning">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq '팀장' }">
																<h5><span class="badge badge-soft-success">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq '실장' }">
																<h5><span class="badge badge-soft-purple">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq '본부장' }">
																<h5><span class="badge badge-soft-info">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq 'CFO' }">
																<h5><span class="badge badge-soft-primary">${hr.empPos }</span></h5>
															</c:if>
															<c:if test="${hr.empPos eq 'CEO' }">
																<h5><span class="badge badge-soft-danger">${hr.empPos }</span></h5>
															</c:if>
														</div>
													</td>
												</tr>
  ...
  selectedNodeIDs.forEach(function (nodeID) {
	  ...
	  $.ajax({
    	  ...
    	  for(let i = 0; i<response.length; i++) {
			  ...
			  var cell3 = $('<td>').html('<div class="d-flex gap-2">' + badge(rowData.empPos) + '</div></div>');

			  function badge(empPos) {
			      switch (empPos) {
			          case '직책 없음':
			              return '<h5><span class="badge badge-soft-secondary">' + empPos + '</span></h5>';
			          case '파트장':
			              return '<h5><span class="badge badge-soft-warning">' + empPos + '</span></h5>';
			          case '팀장':
			              return '<h5><span class "badge badge-soft-success">' + empPos + '</span></h5>';
			          case '실장':
			              return '<h5><span class="badge badge-soft-purple">' + empPos + '</span></h5>';
			          case '본부장':
			              return '<h5><span class="badge badge-soft-info">' + empPos + '</span></h5>';
			          case 'CFO':
			              return '<h5><span class="badge badge-soft-primary">' + empPos + '</span></h5>';
			          case 'CEO':
			              return '<h5><span class="badge badge-soft-danger">' + empPos + '</span></h5>';
			          default:
			              return empPos;
			      }
			  }
