                ...
                <div class="table-responsive">
					<div class="d-flex">
		                		...
		                		<h5>${work.comName } ${work.empName }님의 총 근무시간 : ${totalWork }시간
				                	<c:choose>
									    <c:when test="${totalWork < 40}">
									        <span class="badge bg-danger">미달</span>
									    </c:when>
									    <c:when test="${totalWork >= 40 && totalWork < 52}">
									        <span class="badge bg-success">준수</span>
									    </c:when>
						...
						<div class="p-2 justify-content-end">
	                		<span class="badge rounded-pill bg-info"> </span> 근무
	                		<span class="badge rounded-pill bg-soft-danger"> </span> 휴가
	                                	...
	                                	<div class="d-flex gap-2">
											...
											<c:choose>
											    <c:when test="${workTime == '09:00~18:00' }">
											        <h5><span class="badge bg-info">근무</span></h5>${workTime }
											    </c:when>
											    <c:when test="${workTime == '14:00~18:00' }">
											        <h5><span class="badge bg-soft-danger">휴가</span></h5>${workTime }
											    </c:when>
											    <c:when test="${workTime == '09:00~13:00' }">
											        <h5><span class="badge bg-soft-danger">휴가</span></h5>${workTime }
											    </c:when>
											    <c:otherwise>
											    	<h5><span class="badge bg-info">근무</span></h5>09:00~18:00
											    </c:otherwise>
											</c:choose>
	                                    </div>
	                                </td>
